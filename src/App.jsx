...
// פתרון מוחלט לבעיית חיתוך הפרקים בש"ס. כל מסכת, כל פרק - מדויק!
const EXACT_GEMARA_STARTS = {
  ...
  "שקלים": ["2a", "6b", "9b", "12a", "14a", "16a", "19a", "21a"],
  "יומא": ["2a", "21a", "31a", "39a", "43b", "53b", "61a", "73b"],
  "סוכה": ["2a", "20b", "30a", "42b", "51b"],
  "ביצה": ["2a", "15b", "23b", "28a", "35b"],
  "ראש השנה": ["2a", "22a", "25b", "32a"],
  "תענית": ["2a", "15a", "26a", "30b"],
  "מגילה": ["2a", "17a", "24b", "28b"],
  "מועד קטן": ["2a", "11b", "13b"],
  "חגיגה": ["2a", "11b", "20b"],
  "יבמות": ["2a", "16b", "26a", "36a", "46a", "52b", "60b", "69b", "84a", "87b", "97b", "107a", "112b", "115a", "119a", "121b"],
  "כתובות": ["2a", "15b", "29a", "39a", "41b", "54a", "65a", "71a", "83a", "90a", "95b", "104b", "108a"],
  "נדרים": ["2a", "13a", "24a", "36a", "46a", "50b", "60b", "66a", "76a", "83b", "89b"],
  "נזיר": ["2a", "10a", "15a", "24a", "31a", "34a", "40a", "50a", "59a"],
  "סוטה": ["2a", "14a", "20a", "24a", "27b", "32a", "42b", "44b", "48a"],
  "גיטין": ["2a", "15a", "24a", "32a", "49a", "62b", "74b", "82a", "86a"],
  "קידושין": ["2a", "41a", "58b", "69a"],
  "בבא קמא": ["2a", "17a", "27b", "36b", "46a", "56b", "62b", "87a", "96b", "111b"],
  "בבא מציעא": ["2a", "21a", "33b", "43b", "59b", "74b", "83a", "94a", "101b", "105b"],
  "בבא בתרא": ["2a", "17a", "28b", "52a", "73a", "83b", "95a", "108b", "133b", "156b"],
  "סנהדרין": ["2a", "18a", "24a", "32a", "40a", "42b", "53a", "71a", "73a", "84b", "90a"],
  "מכות": ["2a", "7a", "13b"],
  "שבועות": ["2a", "14b", "20a", "29b", "38b", "41b", "45a", "49a"],
  "עבודה זרה": ["2a", "22a", "40b", "53a", "62a"],
  "הוריות": ["2a", "10a", "12a"],
  "זבחים": ["2a", "15b", "26a", "46b", "53a", "56a", "68b", "83b", "86b", "97a", "101b", "106b", "112b", "115a"],
  "מנחות": ["2a", "11a", "22a", "29a", "36a", "52b", "63a", "72b", "76a", "87a", "90a", "100a", "104b"],
  "חולין": ["2a", "27b", "42a", "54b", "84a", "89b", "93b", "100b", "109a", "113a", "130a", "135b"],
  "בכורות": ["2a", "13a", "17b", "26b", "31b", "37b", "43a", "51b", "57a"],
  "ערכין": ["2a", "7b", "10b", "15a", "18a", "21b", "24a", "28b", "31b"],
  "תמורה": ["2a", "13a", "17a", "21a", "25b", "28b", "31b"],
  "כריתות": ["2a", "8a", "11b", "15b", "19a", "25a"],
  "מעילה": ["2a", "6b", "9a", "12a", "15a", "19a"],
  "נידה": ["2a", "13b", "21a", "32a", "40a", "47b", "56a", "59a", "66b", "69a"]
};

function generateAmudimRange(startStr, endStr, limitDaf) {
    if (!startStr) return [];
    let r = [];
    let sDaf = parseInt(startStr), sAmud = startStr.slice(-1);
    let eDaf = endStr ? parseInt(endStr) : limitDaf, eAmud = endStr ? endStr.slice(-1) : 'b';

    for (let d = sDaf; d <= eDaf; d++) {
        if (d === sDaf && sAmud === 'b') { r.push(`${d}b`); } 
        else if (d === eDaf) { r.push(`${d}a`); if (eAmud === 'b') r.push(`${d}b`); } 
        else { r.push(`${d}a`, `${d}b`); }
    }
    // החסרת עמוד החפיפה
    if (endStr && r.length > 0 && r[r.length-1] === endStr) r.pop();
    return r;
}
...
function getSefariaRefString(cat, bookName, key, tMode, isC, masIdx) {
  ...
  try {
    ...
    // 2. טיפול בספרים מורכבים (כולל המבנה המיוחד של שמירת הלשון)
...
/* ── FLATTEN NESTED SEFARIA ARRAYS (For Shnayim Mikra & Rashi) ── */
const flattenExtra = (heNode, prefix="") => {
    let map = {};
    if(Array.isArray(heNode)) {
        heNode.forEach((item, i) => {
            const num = i + 1;
            ...
            if (Array.isArray(item)) {
                if (item.length > 0 && typeof item[0] === 'string') {
                    ...
                    Object.assign(map, flattenExtra(item, newPrefix));
                }
...
function SefariaReaderSheet({ show, onClose, title, sefariaRef, cat, isTorah, T }) {
  ...
  // Fetch Onkelos and Rashi for Shnayim Mikra mode
  useEffect(() => {
    if (shnayimMode && isTorah && baseRef) {
        Promise.all([
            fetch(`https://www.sefaria.org/api/texts/Onkelos_on_${encodeURIComponent(baseRef)}?context=1&pad=1`),
            fetch(`https://www.sefaria.org/api/texts/Rashi_on_${encodeURIComponent(baseRef)}?context=1&pad=1`)
        ]).then(async ([onkRes, rashiRes]) => {
            const onkData = await (onkRes.ok ? onkRes.json() : {});
            const rashiData = await (rashiRes.ok ? rashiRes.json() : {});
            
            if (onkData.he) setTargumContent(flattenExtra(onkData.he));
            if (rashiData.he) setRashiContent(flattenExtra(rashiData.he));
        }).catch(e => console.error("Error fetching Shnayim Mikra context", e));
    }
  }, [shnayimMode, isTorah, baseRef]);
  ...
  const toggleCommentary = async (item) => {
      ...
      setCommLoading(true);
      try {
          // משתמשים ברפרנס הייחודי לפסוק לפי Sefaria
          ...
          let filteredComm = (d.commentary || []).filter(c => {
              ...
              // סינון מדויק לפי הדרישות כולל שטיינזלץ
              if (cat === "gemara") {
  ...
  return (
    <Sheet show={show} onClose={()=>{onClose(); setRetryCount(0); setActiveSegment(null); setShnayimMode(false);}} title={title} T={T}>
      <div style={{minHeight: 200, maxHeight: '75vh', overflowY: 'auto', paddingRight: 8, direction: 'rtl'}}>
         ...
         {!loading && !error && content.length > 0 && !shnayimMode && (
         ...
         {!loading && !error && content.map((item, i) => {
            ...
            const targum = targumContent[item.fullIdx];
            const rashiArr = rashiContent[item.fullIdx];
            
            return (
            <div key={i} style={{marginBottom: 24, cursor: 'pointer', padding: '0 8px', borderRight: isActive && !shnayimMode ? `4px solid ${T.gold||GOLD}` : '4px solid transparent', transition: 'all 0.2s'}} onClick={() => !shnayimMode && toggleCommentary(item)}>
               
               {/* תצוגת שניים מקרא ואחד תרגום (כולל רש"י למטה) */}
               {item.he && shnayimMode ? (
                   <>
                       <p style={{fontSize: zoom, lineHeight: 1.6, color: T.navy, fontFamily: "'Frank Ruhl Libre', serif", fontWeight: 500, textAlign: 'justify', margin: "0 0 6px 0"}}>
                       ...
                       {targum && (
                           <p style={{fontSize: Math.max(14, zoom * 0.85), lineHeight: 1.5, color: T.muted, fontFamily: "'Frank Ruhl Libre', serif", textAlign: 'justify', margin: "0 0 12px 0", paddingRight: 10, borderRight: `2px solid ${T.border}`}}>
                              <span style={{fontWeight: 700}}>תרגום: </span><span dangerouslySetInnerHTML={{__html: targum}} />
                           </p>
                       )}
                       {rashiArr && Array.isArray(rashiArr) && (
                           <div style={{background: T.dark ? '#1A2436' : '#FBF5E0', borderRadius: 10, padding: "10px 14px", marginTop: 8}}>
                               <div style={{fontSize: Math.max(12, zoom * 0.7), fontWeight: 800, color: T.gold||GOLD, marginBottom: 4}}>רש״י</div>
                               {rashiArr.map((rc, rIdx) => (
                                  <p key={rIdx} style={{fontSize: Math.max(13, zoom * 0.75), color: T.navy, lineHeight: 1.5, fontFamily: "'Frank Ruhl Libre', serif", margin: "0 0 6px 0"}} dangerouslySetInnerHTML={{__html: rc}} />
                               ))}
                           </div>
                       )}
                   </>
               ...
               {/* חלונית מפרשים לתצוגה רגילה */}
               {isActive && !shnayimMode && (
